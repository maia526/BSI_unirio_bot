...
def start(update: Update, context: CallbackContext):
    update.message.reply_text("O BSI_unirio_bot te dá boas vindas! Digite /ajuda para ver os comandos disponíveis.")
...
updater.dispatcher.add_handler(CommandHandler('calendario', retorna_lista_datas))

updater.dispatcher.add_handler(MessageHandler(Filters.text, desconhecido))
updater.dispatcher.add_handler(MessageHandler(Filters.command, desconhecido)) #filtra comandos desconhecidos

updater.dispatcher.add_handler(MessageHandler(Filters.text, texto_desconhecido)) #filtra mensagens desconhecidas
